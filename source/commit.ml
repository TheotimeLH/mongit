...
(* Voir si on garde l'ancien *)
  ...
  (* MINOR : On note dans le fichier de commit les différences *)
    ...
    f old_key new_key (List.length old_in_new) ;
...
let compile_commit () = (* -> sha du commit *)
  ...
  let nb_nothing = nb_f_ch - (List.length f_to_ch) in
  ...
  then (printf "Nothing to commit, use mg -add first.\n"; "")
    ...
    List.iter (do_f_cr cch) f_to_cr ;
    List.iter (do_f_ch cch) f_to_ch ;
    List.iter (do_d_mv cch) d_to_mv_tree ;
    List.iter (do_f_mv cch) f_to_mv_tree ;
    List.iter (do_d_rm cch) d_to_rm_tree ;
    List.iter (do_f_rm cch) f_to_rm_tree ;
