...
let rec add_file dr_tree f = 
  (* /!\ apres Unix.realpath et non déjà présent !*)
  let dir = Filename.dirname f in
  let hdir = Filename.concat dr_tree (Outils.sha_name dir) in 
...
and add_dir dr_tree dir =
  let parent = Filename.dirname dir in
  let hparent = Filename.concat dr_tree (Outils.sha_name parent) in
